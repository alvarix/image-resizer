...
// ---- Helpers ----
/** Return the file name from a full path (with extension). */
function basename(p: string): string {
	return p.split("/").pop() ?? p;
}

/** Helper to extract the stem (name without extension) from a path. */
function parse(p: string): { name: string } {
	const base = basename(p);
	const dot = base.lastIndexOf(".");
	return { name: dot > 0 ? base.slice(0, dot) : base };
}
...
function renderPresets(): void {
	if (state.presets.length === 0) {
		presetListEl.innerHTML =
			'<li class="preset-empty">No presets. Click + to add one.</li>';
		renderRunBtn();
		return;
	}
	...
	presetListEl
		.querySelectorAll<HTMLInputElement>(".preset-check")
		.forEach((cb) => {
	...
	// Inline param inputs: update preset, auto-name, and refresh both presets + file list
	presetListEl
		.querySelectorAll<HTMLInputElement>(".param-size")
		.forEach((inp) => {
			inp.addEventListener("change", () => {
				...
				if (preset) {
					...
					preset.maxLongestSide = Math.max(
						1,
	...
	presetListEl
		.querySelectorAll<HTMLInputElement>(".param-colors")
		.forEach((inp) => {
			inp.addEventListener("change", () => {
				const id = (inp.closest("[data-id]") as HTMLElement).dataset.id!;
				const preset = state.presets.find((p) => p.id === id);
	...
	presetListEl
		.querySelectorAll<HTMLInputElement>(".param-quality")
		.forEach((inp) => {
...
function editorCallbacks(_currentId: string) {
	return {
...
/** Persist presets and refresh both preset list and file list UI. */
		...
		.map(
          ...
          <div class="file-info">
            ...
            <div class="file-outputs">
              ${enabledPresets
								.map((p) => {
									const ext = outputExtension(p);
									const outName = `${parse(f).name}-${p.name}.${ext}`;
									return `<span class="file-output">→ ${outName}</span>`;
								})
								.join("")}
            </div>
		...
		jobs.push(
			(async () => {
				try {
					...
					const img = fileListEl.querySelector<HTMLImageElement>(
						`.file-row[data-idx="${i}"] .file-thumb`,
					);
...
runBtn.addEventListener("click", async () => {
	...
	const unsubscribe = window.api.onPipelineProgress((evt: ProgressEvent) => {
		...
		if ((e as { type: string }).type === "item") {
			...
			const statusEl2 =
				fileIdx >= 0
					? fileListEl.querySelector<HTMLElement>(
							`.file-row[data-idx="${fileIdx}"] .file-statuses`,
						)
					: null;
			if (statusEl2) {
				...
				if (item.status === "ok" && item.outPath) {
					...
					pill.addEventListener("click", () =>
						window.api.showInFinder(outPath),
					);
				}
...
function formatRunEntry(r: RunEntry): string {
	...
	const presetList = r.presetNames?.length
		? `<div class="run-presets">${r.presetNames.map((n) => escHtml(n)).join(" · ")}</div>`
		: "";
	return `<li class="run-entry"><span class="run-time">${date} ${time}</span><span class="run-summary">${summary}${presetList}</span></li>`;
}
